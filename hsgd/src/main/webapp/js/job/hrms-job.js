...
function lookItem(id) {
    ...
    //2.加载查看页面
    //2-1.查看页面是请求服务器得到的
    $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/job/look/" + id + "' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
}
...
function updateItem(id) {
    //1.获得查看行id
    console.log('职位id', id);
    $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/job/update?id=" + id + "' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
...
function deleteItem(id) {
    if(confirm('确认删除吗？')) {
        console.log('职位id', id);
        $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/job/del?id=" + id + "' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
    }
}
...
function query() {
    var name = $('#name').val();
    $.get(basePath + '/job/list',{name:name}, function (data) {
        dataHandler(data);
    }, 'json');
}
